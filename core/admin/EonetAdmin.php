...
class EonetAdmin extends Eonet
{
    ...
    public function ajaxSaveSettings() {
        ...
        if($value || $eonet_settings == get_option($option_field)) {
            ...
            $response['title'] = __('Done, everything is safe !', 'eonet-frontend-publisher');
            $response['content'] = __('Your settings have been saved successfully.', 'eonet-frontend-publisher');
        } else {
            $response['status'] = 'error';
            $response['title'] = __('Something went wrong...', 'eonet-frontend-publisher');
            $response['content'] = __('We\'ve not been able to save your settings, please try again later.', 'eonet-frontend-publisher');
        }
    ...
    public function ajaxResetSettings() {
        ...
        if($value) {
            $response['status'] = 'success';
            $response['title'] = __('Done, settings have been reset !', 'eonet-frontend-publisher');
    ...
    public function adminMenus(){
        // main page :
        add_menu_page(
            __( 'Eonet Dashboard', 'eonet-frontend-publisher' ),
            __( 'Eonet','eonet-frontend-publisher' ),
            'manage_options',
            'eonet',
    ...
    static public function getThemes() {
        $themes = array(
            array(
                'name' => __('Woffice Intranet/Extranet theme', 'eonet-frontend-publisher'),
                ...
                'premium' => true,
                'features' => array(
                    __('Projects Management','eonet-frontend-publisher'),
                    __('Complete Wiki/Knowledge base','eonet-frontend-publisher'),
                    __('Members map & directory','eonet-frontend-publisher'),
                    __('Responsive layout (mobile & tablet)','eonet-frontend-publisher'),
                    __('Cover picture','eonet-frontend-publisher'),
                    __('Private / Public pages','eonet-frontend-publisher'),
                    __('Files Manager','eonet-frontend-publisher'),
                    __('Events calendar','eonet-frontend-publisher'),
                )
            )
        );
