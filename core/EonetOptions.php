...
class EonetOptions {
	...
	static public function getSubmit($type = 'new', $label = '') {
		...
		if($type == 'delete') {
			$icon = 'fa-trash-o';
			$text = __('Delete', 'eonet-frontend-publisher');
		}
		elseif ($type == 'new') {
			$icon = 'fa-pencil-square-o';
			$text = __('New', 'eonet-frontend-publisher');
		} else {
			$icon = 'fa-check-circle';
			$text = __('Save', 'eonet-frontend-publisher');
		}
	...
	static protected function renderFieldTypeIconPicker( $option, $option_saved ) {
		...
		ob_start();
		?>
		<div id="eo_field_<?php echo $option['name']; ?>" class="eo_field_iconpicker_wrapper">

			<div class="eo_field_iconpicker_icon_selected_wrapper">
				<div class="eo_field_iconpicker_icon_selected"><i class="<?php echo $icon_value; ?>"></i></div>
			...
			<div class="eo_field_iconpicker_icons_details">
				<label class="eo_label_for_select">
					...
					<select class="eo_field_iconpicker_group_select">
						...
						foreach ( $icons_array['groups'] as $group_key => $group_name ) {
							echo '<option value="' . esc_attr__( $group_key ) . '" >' . esc_html__( $group_name ) . '</option>' . "\n";
						}
						?>
					</select>
				</label>
				...
				<div class="eo_field_iconpicker_icons_grid_container">
					...
					<?php foreach ( $icons_array['icons'] as $group_key => $group ) : ?>
						<div class="eo_field_iconpicker_icons_grid_panel " data-eo-icons-group="<?php echo $group_key; ?>">
							...
							<div class="eo_field_iconpicker_icons_grid_panel_icons eo_clearfix">
								<?php foreach ( $group as $icon_class ): ?>
									...
									?>
									<a href="javascript:void(0)" class="eo_field_iconpicker_icons_grid_element"><i class="<?php echo $icon_class; ?>"></i></a>
								<?php endforeach; ?>
							</div>
						</div>
					<?php endforeach; ?>
			...
			<select name="eo_field_<?php echo $option['name']; ?>" class="eo_field_iconpicker_select">
				...
				foreach ( $icons_array['icons'] as $group => $icons ) {
					echo '<optgroup label="' . esc_attr__( $group ) . '">' . "\n";
					foreach ( $icons as $key => $label ) {
						$class_key = $label;
						echo '<option value="' . esc_attr__( $class_key ) . '" ' . ( strcmp( $class_key, $icon_value ) === 0 ? 'selected' : '' ) . '>' . esc_html__( $label ) . '</option>' . "\n";
					}
					echo '</optgroup>' . "\n";
				}
				?>
    ...
    static protected function renderFieldTypeUpload( $option, $option_saved ) {
        ...
        $html .= '<div class="eo_field_upload_wrapper">';
            ...
            // Button :
            $html .= '<button data-eo-name="eo_field_' . $option['name'] . '" name="eo_field_' . $option['name'] . '_trigger" class="eo_btn_upload_wp eo_btn eo_btn_default"><i class="fa fa-picture-o"></i> '.__('Choose an Image', 'eonet-frontend-publisher').'</button>';
        $html .= '</div>';
	 ...
	 *  'name'      => 'field_id',
	 *  'type'      => 'typography',
	 *  'label'     => __('Select a font', 'eonet'),
	 *  'desc'      => __('Your desc here', 'eonet'),
	 *  'font-family' => true,
	 *  'font-size => true,
	 *  'val'       => array(
