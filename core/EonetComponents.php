...
class EonetComponents{
    ...
    public function addSetting($links) {
        ...
        $link = '<a href="' . admin_url( 'plugins.php?page=eonet&eo_settings='.$this->current_component ) . '">'. __('Settings','eonet-frontend-publisher') .'</a>';
    ...
    public function ajaxStateChange() {
        ...
        if(!empty($method) && !empty($component)) {
            ...
            if($method == "deactivate") {
                ...
                $response['title'] = __('Extension has been deleted !', 'eonet-frontend-publisher');
                $response['content'] = __('The page will be refreshed in a few seconds...', 'eonet-frontend-publisher');
            }
            if($method == "activate") {
                $installed = $this->activate($component);
                if($installed == false){
                    $response['status'] = 'error';
                    $response['title'] = __('We couldn\'t install it...', 'eonet-frontend-publisher');
                    $response['content'] = __('Please try from your plugin page.', 'eonet-frontend-publisher');
                }
                /*$response['status'] = 'success';
                $response['title'] = __('We\'re installing the extension !', 'eonet-frontend-publisher');
                $response['content'] = __('The page will be refreshed in a few seconds...', 'eonet-frontend-publisher');*/
            }
        } else {
            $response['status'] = 'error';
            $response['title'] = __('Something went wrong...', 'eonet-frontend-publisher');
    ...
    static public function getComponents($slug = '') {
        ...
        $components['live-search'] = array(
            'name' => __('Live Search', 'eonet-frontend-publisher'),
            'version' => '1.0.0',
            'icon' => 'ion-ios-search',
            'premium' => false,
            'description' => __('Enhance your Wordpress search bar with customizable live search through all your post types and members.', 'eonet-frontend-publisher'),
        );

        $components['frontend-publisher'] = array(
            'name' => __('Frontend Publisher', 'eonet-frontend-publisher'),
            'version' => '1.0.0',
            'icon' => 'ion-ios-compose-outline',
            'premium' => false,
            'description' => __('Allow yourself, your team or your user to edit and publish any Wordpress post or page.', 'eonet-frontend-publisher'),
        );

	    $components['manual-user-approve'] = array(
		    'name' => __('Manual User Approve', 'eonet-frontend-publisher'),
		    'version' => '1.0.0',
		    'icon' => 'ion-person-add',
		    'premium' => false,
		    'description' => __('Set a pending status to user regsitered on your site, you can manually approve or deny them.', 'eonet-frontend-publisher'),
	    );

        $components['live-notifications'] = array(
            'name' => __('Live Notifications', 'eonet-frontend-publisher'),
            'version' => '1.0.0',
            'icon' => 'ion-ios-loop',
            'premium' => false,
            'description' => __('Enables live notifications for all your users to get better interactions within your site.', 'eonet-frontend-publisher'),
        );

        $components['project-manager'] = array(
            'name' => __('Project Manager', 'eonet-frontend-publisher'),
            'version' => '0.0.0',
            'icon' => 'ion-briefcase',
            'premium' => false,
            'description' => __('Enables a complete project management tool for your site.', 'eonet-frontend-publisher'),
        );

        $components['social-connect'] = array(
            'name' => __('Social Connect', 'eonet-frontend-publisher'),
            'version' => '0.0.0',
            'icon' => 'ion-ios-cloud-download-outline',
            'premium' => false,
            'description' => __('Allow your visitor to login or register using Facebook, Twitter, Slack or Linkedin.', 'eonet-frontend-publisher'),
        );
